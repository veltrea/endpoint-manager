...
interface IAIProviderNode {
  id:              string;         // ノード固有ID。例: "local-http://192.0.2.22:1234/v1"
  type:            'cloud' | 'local';
  providerType:    ProviderType;   // 後述
  providerUrl:     string;         // エンドポイントのベースURL。例: "http://192.0.2.22:1234/v1"
  label?:          string;         // 任意の表示名
...
// 例: { "planner": "local-http://192.0.2.22:1234/v1", "coder": "..." }
```
...
// レスポンス
{
  ...
  "ok": true,
  "data": {
    "id": "local-http://192.0.2.22:1234/v1",
    "providerUrl": "http://192.0.2.22:1234/v1",
...
// レスポンス
{
  ...
  "ok": true,
  "data": {
    "planner": "local-http://192.0.2.22:1234/v1",
...
```json
// リクエスト
{ "id": "8", "type": "remove_endpoint", "endpointId": "local-http://192.0.2.50:1234/v1" }
...
// リクエスト
{
  ...
  "type": "assign_role",
  "role": "coder",
  "endpointId": "local-http://192.0.2.22:1234/v1"
}
...
```json
{
  ...
  "data": {
    "nodeId": "local-http://192.0.2.22:1234/v1",
    "state": "Offline"
  }
}
```
...
{
  "event": "node_removed",
  "data": { "nodeId": "local-http://192.0.2.22:1234/v1" }
}
```
...
async function getCodeCompletionEndpoint(): Promise<string> {
  ...
  return response.data.providerUrl;  // "http://192.0.2.22:1234/v1"
}
