...
Mac mini（LAN 内のテスト機）でのクロスプラットフォームテスト:

```bash
# WoL で起動
python3 -c "import socket; mac='<MAC_ADDRESS>'; ..."

# テストを実行
ssh <user>@<mac-host> 'cd /path/to/endpoint-manager && npm test'
```

Windows（LAN 内のテスト機）での動作確認:
```bash
ssh -o ProxyJump=none <user>@<windows-host> 'cd /path && npm test'
```
